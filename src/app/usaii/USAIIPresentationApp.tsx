...
function IconBadge({ icon: Icon, active = false, large = false }: { icon: LucideIcon; active?: boolean; large?: boolean }) {
  return (
    <div
      ...
      } ${active ? "border-accent bg-accent text-white" : "border-border bg-bg-light text-secondary-dark"}`}
    >
...
function QuoteBar({ children, support }: { children: ReactNode; support?: ReactNode }) {
  return (
    <Card className="flex items-start gap-5 p-5">
      ...
      <div>
        <p className="text-[clamp(1.35rem,2.2vw,2rem)] leading-[1.16] tracking-[-0.04em] text-ink">{children}</p>
        {support ? <p className="mt-2 text-[clamp(.9rem,1.2vw,1.05rem)] leading-snug text-muted">{support}</p> : null}
      </div>
    </Card>
  );
}
...
function SlideCanvas({
  ...
  return (
    <section className="flex h-full w-full flex-col overflow-hidden rounded-[30px] bg-bg-light p-8 text-ink md:p-10">
      <div className="flex items-start justify-between gap-6">
        <div>
          <Kicker>USAII AI Expert Talks</Kicker>
      ...
      <div className="mt-6 max-w-[980px]">
        <h1 className="font-display text-[clamp(2.2rem,4.2vw,4.3rem)] font-normal leading-[1.02] tracking-[-0.055em] text-ink">
          {title}
        </h1>
        {subtitle ? (
          <p className="mt-3 max-w-[850px] text-[clamp(1.05rem,1.8vw,1.7rem)] leading-[1.3] tracking-[-0.025em] text-muted">
            {subtitle}
          </p>
        ) : null}
      </div>
    ...
    <SlideCanvas number={1} title="How to Turn AI Agents Into Governed Digital Products" subtitle="Enterprise Agent Operating Model">
      <div className="mt-8 grid h-[calc(100%-2rem)] grid-cols-[0.9fr_1.1fr] gap-8">
        <div className="flex flex-col justify-end pb-4">
          <QuoteBar support="A practical product operating model for delegated autonomy.">
            Autonomy is a product decision, not a model property.
          </QuoteBar>
          <p className="mt-5 text-[clamp(1rem,1.35vw,1.2rem)] tracking-[-0.02em] text-secondary-dark">Jim Markunas</p>
        </div>

        <div className="relative flex items-center justify-center">
          <div className="absolute h-[300px] w-[300px] rounded-full border border-dashed border-[#cfd3d8]" />
          <Card className="relative z-20 flex h-[170px] w-[220px] flex-col items-center justify-center p-5 text-center">
            <IconBadge icon={BrainCircuit} active large />
            <h2 className="mt-3 text-[26px] tracking-[-0.04em]">AI Agent</h2>
            <p className="mt-1 text-[15px] leading-snug text-muted">delegated authority wrapped in controls</p>
          </Card>
          ...
          {agentModel.map((item, index) => {
            ...
            return (
              <Card key={item.letter} className={`absolute z-10 flex w-[170px] items-center gap-3 p-3 ${positions[index]}`}>
                <IconBadge icon={item.icon} />
                <div>
                  <div className="text-xs font-medium uppercase tracking-[0.2em] text-accent">{item.letter}</div>
                  <div className="text-[15px] leading-tight tracking-[-0.025em]">{item.name}</div>
                </div>
              </Card>
            );
          })}
...
function AgentDefinitionSlide() {
  const steps = [
    ...
    { title: "Model + tools", body: "Invokes capabilities when directed", icon: Layers3 },
    {
      ...
      body: "Pursues an objective, evaluates state, chooses among permitted actions, and can change a business system",
      icon: Target,
      active: true,
    },
  ];
  ...
  return (
    <SlideCanvas number={2} title="What Makes an Enterprise AI Agent?" subtitle="From response to delegated consequence">
      <div className="mt-7 grid grid-cols-[1fr_245px] gap-6">
        <div className="grid grid-cols-4 gap-4">
          {steps.map((step, index) => (
            <div key={step.title} className="relative">
              {index < steps.length - 1 ? (
                <ArrowRight className="absolute -right-4 top-1/2 z-20 h-5 w-5 -translate-y-1/2 text-[#9ca3af]" />
              ) : null}
              <Card className={`relative flex min-h-[285px] flex-col items-center justify-center p-5 text-center ${step.active ? "border-accent ring-1 ring-accent" : ""}`}>
                ...
                <h2 className={`mt-5 text-[clamp(1.2rem,1.7vw,1.6rem)] leading-tight tracking-[-0.04em] ${step.active ? "text-accent" : "text-ink"}`}>
                  {step.title}
                </h2>
                <p className="mt-3 text-[clamp(.85rem,1.15vw,1rem)] leading-snug text-muted">{step.body}</p>
              </Card>
            </div>
          ))}
        </div>
        <Card className="flex flex-col justify-center p-6">
          ...
          <p className="mt-3 text-[clamp(1.4rem,2vw,1.9rem)] leading-[1.13] tracking-[-0.05em]">
      ...
      <div className="mt-5 flex gap-3">
        {[
          ...
          <div key={label as string} className="flex items-center gap-2 rounded-pill border border-border bg-white px-4 py-2 text-sm text-secondary-dark">
  ...
  return (
    <SlideCanvas number={3} title="The Enterprise Agent Operating Model" subtitle="Six product decisions for governed autonomy">
      <div className="mt-7 grid grid-cols-3 gap-4">
        {agentModel.map((item) => (
          <Card key={item.letter} className="flex min-h-[145px] items-center gap-5 p-5">
            <IconBadge icon={item.icon} />
            <div className="min-w-0">
              <div className="flex items-baseline gap-3">
                <span className="text-[52px] font-medium leading-none tracking-[-0.08em] text-accent">{item.letter}</span>
                <h2 className="text-[clamp(1.1rem,1.55vw,1.45rem)] leading-tight tracking-[-0.035em]">{item.name}</h2>
              </div>
              <div className="my-2 h-px w-full bg-border" />
              <p className="text-[clamp(.8rem,1vw,.95rem)] leading-snug text-secondary-dark">{item.prompt}</p>
            </div>
          </Card>
        ))}
      </div>
      <div className="mt-5">
        <QuoteBar>The model is a component. The agent is the product.</QuoteBar>
      </div>
    </SlideCanvas>
  );
}
...
function AuthoritySlide() {
  ...
  return (
    <SlideCanvas number={4} title="Authority Is a Ladder, Not a Toggle" subtitle="Assign autonomy action by action">
      <div className="mt-7 grid grid-cols-[1fr_235px] gap-6">
        <div className="flex items-end gap-3">
          {levels.map((level, index) => (
            <Card
              key={level.title}
              className={`relative flex flex-1 flex-col items-center justify-center p-4 text-center ${index === levels.length - 1 ? "border-accent ring-1 ring-accent" : ""}`}
              style={{ height: `${190 + index * 28}px` }}
            >
              ...
              <IconBadge icon={level.icon} active={index === levels.length - 1} />
              <h2 className="mt-3 text-[clamp(1.05rem,1.35vw,1.3rem)] tracking-[-0.04em]">{level.title}</h2>
              <div className="my-2 h-px w-20 bg-border" />
              <p className="text-[clamp(.75rem,.95vw,.9rem)] leading-snug text-muted">{level.body}</p>
            </Card>
          ))}
        </div>
        ...
        <Card className="self-end p-5">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-xl tracking-[-0.04em]">Decision test</h3>
          </div>
          <ul className="mt-5 space-y-3 text-[15px] text-secondary-dark">
      ...
      <p className="mt-5 text-center text-[clamp(1.1rem,1.55vw,1.35rem)] italic tracking-[-0.025em] text-secondary-dark">
      ...
      <div className="mt-7 grid grid-cols-7 gap-3">
        {chain.map((item, index) => (
          <div key={item.title} className="relative">
            ...
            <Card className={`flex min-h-[210px] flex-col items-center justify-center p-4 text-center ${item.active ? "border-accent" : ""}`}>
              <IconBadge icon={item.icon} active={item.active} />
              <h2 className={`mt-3 text-[clamp(1rem,1.2vw,1.2rem)] tracking-[-0.035em] ${item.active ? "text-accent" : ""}`}>
                {item.title}
              </h2>
              <p className="mt-2 text-[clamp(.7rem,.82vw,.8rem)] leading-snug text-muted">{item.body}</p>
            </Card>
          </div>
        ))}
      </div>
      <div className="mt-5 ml-auto max-w-[680px]">
        <QuoteBar support="More authority should generally mean more observability.">
...
function IntegrationSlide() {
  ...
  const nodes = [
    { title: "Identity", body: "Who the agent is", icon: UserCheck },
    { title: "Permissions", body: "What it may access", icon: Lock },
    { title: "Systems of record", body: "Source of truth", icon: Database },
    { title: "CRM", body: "Customer data", icon: Users },
    { title: "Billing", body: "Invoices and accounts", icon: ClipboardCheck },
    { title: "Eligibility", body: "Policies and rules", icon: ShieldCheck },
    { title: "APIs", body: "Integration layer", icon: Layers3 },
    { title: "Workflows", body: "Execute and orchestrate", icon: Workflow },
  ];
  ...
  return (
    <SlideCanvas number={6} title="Integration in the Real Enterprise" subtitle="Treat the agent as a new actor inside the architecture">
      <div className="mt-6 grid grid-cols-[300px_1fr] gap-7">
        <Card className="h-fit p-5">
          <div className="text-[11px] font-medium uppercase tracking-[0.25em] text-muted">Principles</div>
          <div className="mt-4 space-y-3">
            {principles.map((item) => (
              <div key={item.label} className="flex items-center gap-3 border-b border-border pb-3 last:border-b-0">
                <IconBadge icon={item.icon} />
                <span className="text-[17px] tracking-[-0.025em]">{item.label}</span>
              </div>
            ))}
          </div>
        </Card>

        <div className="relative flex min-h-[355px] items-center justify-center">
          <div className="absolute h-[290px] w-[520px] rounded-full border border-dashed border-[#cfd3d8]" />
          <Card className="relative z-20 flex h-[150px] w-[190px] flex-col items-center justify-center p-4 text-center">
            <IconBadge icon={BrainCircuit} active large />
            <h2 className="mt-2 text-2xl tracking-[-0.04em]">AI Agent</h2>
            <p className="text-sm text-muted">purpose-built and bounded</p>
          </Card>
          ...
          {nodes.map((item, index) => {
            ...
            return (
              <Card key={item.title} className={`absolute z-10 w-[145px] p-3 text-center ${positions[index]}`}>
                <IconBadge icon={item.icon} />
                <h3 className="mt-2 text-[15px] leading-tight tracking-[-0.025em]">{item.title}</h3>
                <p className="mt-1 text-xs leading-snug text-muted">{item.body}</p>
              </Card>
            );
          })}
        </div>
      ...
      <div className="mt-5 grid grid-cols-[1.1fr_1fr] gap-4">
        ...
        <Card className="flex items-center gap-3 p-5 text-[15px] leading-snug text-muted">
          <Database className="h-5 w-5 shrink-0 text-accent" />
          AI doesn’t resolve conflicting systems of record by being confident.
        </Card>
      </div>
    </SlideCanvas>
  );
...
function TransferSlide() {
  ...
  const packageItems = ["Objective", "Context", "Checks performed", "Actions attempted", "Reason for escalation", "Decision required"];

  return (
    <SlideCanvas number={7} title="Transfer & Escalation" subtitle="Know when authority ends — and what context must transfer">
      <div className="mt-6 grid grid-cols-[350px_1fr] gap-7">
        <div className="flex flex-col justify-center">
          <div className="text-5xl text-accent">“</div>
          <p className="mt-2 text-[clamp(1.75rem,2.8vw,2.7rem)] leading-[1.12] tracking-[-0.055em]">
            A human-in-the-loop design fails if the human has to start the loop over.
          </p>
          <div className="mt-6 h-px w-full bg-border" />
          <p className="mt-4 text-[clamp(1rem,1.35vw,1.2rem)] leading-snug text-muted">
            The goal isn’t zero escalations. The goal is correct escalations.
          </p>
        </div>
        ...
        <div>
          <div className="grid grid-cols-5 gap-3">
            {triggers.map((item) => (
              <Card key={item.label} className="p-3 text-center">
                <IconBadge icon={item.icon} />
                <p className="mt-2 text-sm leading-tight tracking-[-0.02em]">{item.label}</p>
              </Card>
            ))}
          </div>
          ...
          <Card className="mt-5 border-accent p-5 ring-1 ring-accent">
            <div className="flex items-center justify-center gap-3">
              <IconBadge icon={ClipboardCheck} active />
              <div>
                <h3 className="text-[24px] tracking-[-0.04em] text-accent">Handoff package</h3>
                <p className="text-sm text-muted">Everything the next authority needs</p>
              </div>
            </div>
            <div className="mt-4 grid grid-cols-6 divide-x divide-border rounded-[16px] border border-border bg-white p-3">
              {packageItems.map((item) => (
                <div key={item} className="px-2 text-center text-xs leading-tight text-secondary-dark">
                  {item}
                </div>
              ))}
            </div>
  ...
  return (
    <SlideCanvas number={8} title="How to Measure Real Value" subtitle="Activity is not value">
      <div className="mt-7 grid grid-cols-3 gap-5">
        {buckets.map((bucket) => (
          <Card key={bucket.title} className="min-h-[285px] p-6 text-center">
            <IconBadge icon={bucket.icon} active large />
            <h2 className="mt-4 text-[clamp(1.35rem,1.8vw,1.65rem)] tracking-[-0.045em]">{bucket.title}</h2>
            <div className="mx-auto my-4 h-px w-3/4 bg-border" />
            <ul className="space-y-3 text-left text-[clamp(.9rem,1.15vw,1.05rem)] text-secondary-dark">
      ...
      <div className="mt-5">
        <QuoteBar support="Time saved only becomes value when the organisation converts it into an outcome.">
          Productivity isn’t the outcome. Business performance is.
        </QuoteBar>
      </div>
    </SlideCanvas>
  );
    ...
    <SlideCanvas number={9} title="Who Is Accountable?" subtitle="Execution can be delegated. Accountability cannot.">
      <div className="mt-7 grid grid-cols-[0.9fr_1.1fr] gap-7">
        <div className="flex flex-col justify-end">
          <QuoteBar support="Root cause can be distributed. Accountability cannot be anonymous.">
            Delegating an action to software does not delegate accountability away from the organisation.
          </QuoteBar>
        </div>
        ...
        <div className="flex flex-col gap-3">
          {layers.map((layer, index) => (
            <Card key={layer.title} className={`flex items-center gap-4 p-4 ${layer.active ? "border-accent" : ""}`}>
              <div className={`grid h-14 w-14 place-items-center rounded-[16px] ${layer.active ? "bg-accent text-white" : "bg-bg-light text-muted"}`}>
                <layer.icon className="h-6 w-6" />
              </div>
              <div>
                <h3 className={`text-[clamp(1.25rem,1.55vw,1.5rem)] tracking-[-0.04em] ${layer.active ? "text-accent" : ""}`}>
                  {layer.title}
                </h3>
                <p className="text-[clamp(.8rem,1vw,.95rem)] text-muted">{layer.body}</p>
              </div>
    ...
    <SlideCanvas number={10} title="Production-Ready Agent Checklist" subtitle="Six questions every team should answer before granting autonomy">
      <div className="mt-7 grid grid-cols-3 gap-4">
        {questions.map((question, index) => (
          <Card key={question.text} className="flex min-h-[110px] items-center gap-4 p-5">
            <IconBadge icon={question.icon} />
            <div className="h-14 w-px bg-border" />
            <div>
              <div className="mb-1 text-xs uppercase tracking-[0.18em] text-muted">{String(index + 1).padStart(2, "0")}</div>
              <h2 className="text-[clamp(1.15rem,1.45vw,1.4rem)] leading-[1.05] tracking-[-0.045em]">{question.text}</h2>
            </div>
          </Card>
        ))}
      </div>
      <div className="mt-5">
        <QuoteBar>
          If you can’t answer these clearly, you don’t have a <span className="text-accent">governed enterprise agent yet</span>. You have an <span className="text-accent">experiment with access to production</span>.
        </QuoteBar>
      </div>
    </SlideCanvas>
  );
