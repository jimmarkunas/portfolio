...
export function Slide07() {
  ...
  return <Stage>
    <B geometry={g.matrix} bg="#121518" border="1px solid #3a3f44" radius={14} style={{ borderBottom: "0 solid transparent" }}/><B geometry={g.decisionColumn} bg="#15181b" border="1px solid #3a3f44" radius={14} style={{ borderBottom: "0 solid transparent" }}/><B geometry={g.highlightColumn} bg="rgba(36,14,29,.92)" border={`1px solid ${MAGENTA}`} style={{ borderBottom: "0 solid transparent" }}/>
    ...
    {cols.map(([title, desc], i) => { const c = g.columnHeading[i], b = g.columnBody[i]; return <div key={title}><T geometry={c} weight={800} color={i === 0 ? MAGENTA : WHITE}>{title}</T><T x={b.x} y={b.y} w={b.width} size={b.size} line={b.line}>{desc}</T></div>; })}
    ...
    <B geometry={g.takeawayAccent} bg={MAGENTA}/><T geometry={g.takeaway} weight={600}>IF YOU CAN&apos;T IDENTIFY ONE OF THESE OUTCOMES, YOU DON&apos;T HAVE A PRODUCT.</T>
  </Stage>;
}
