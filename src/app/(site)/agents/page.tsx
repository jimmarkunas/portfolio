    ...
    <main data-gpme-route="agents" data-gpme-deploy-sha={process.env.NEXT_PUBLIC_DEPLOY_SHA} className="min-h-full overflow-x-hidden bg-[#F3F3F3]">
      <Container className="pb-16 pt-8 md:pb-20 md:pt-10 lg:pb-[100px] lg:pt-[42px]">
        <div className="flex flex-col gap-14 md:gap-16 lg:gap-20">
              ...
              <section className="grid gap-5 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)] lg:items-start lg:gap-x-12 xl:gap-x-16">
                <div className="lg:col-start-1 lg:row-start-1">
                  ...
                  <div className="mt-7 flex flex-wrap gap-3">
                    <a href="#readiness-check" className="button-primary">{agentsContent.hero.primaryCta}</a>
                    <a href="#agents-resources" className="button-secondary">{agentsContent.hero.secondaryCta}</a>
                  </div>
                </div>
          ...
          <section id="agents-resources" className="scroll-mt-24 rounded-[10px] bg-[#ECECEC] px-5 py-7 md:px-8 md:py-9 lg:px-10 lg:py-10">
            <EyebrowPill className="w-fit bg-white" labelClassName="type-p5 text-[#222222]">{agentsContent.resources.label}</EyebrowPill>
            ...
            <div className="mt-7 grid gap-7 lg:grid-cols-[minmax(0,0.75fr)_minmax(0,1.25fr)] lg:gap-10">
              ...
              <ul className="space-y-3">{agentResources?.assets.map((asset, index) => <li key={asset.href} className="rounded-[10px] bg-white px-4 py-4 outline outline-1 outline-black/5"><div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div><h3 className="type-p2 text-[#2A2A2A]">{asset.title}</h3><p className="type-p4 mt-1 text-[#4B5154]">{asset.kind}</p></div><div className="flex flex-wrap items-center gap-3">{index === 0 ? <TrackedExternalLink href={asset.href} label={`Download ${asset.title}`} location="freebies_download" download={asset.fileName} target="_self" className="button-primary min-h-[44px] px-4 text-[15px] hover:!border-[#447ACB] hover:!bg-[#447ACB] hover:!text-white" ariaLabel={`Download ${asset.title}`}><Download className="h-4 w-4" aria-hidden="true" /><span>{agentsContent.resources.downloadCta}</span></TrackedExternalLink> : <FreebiesDownloadButton href={asset.href} title={asset.title} fileName={asset.fileName} />}<TrackedExternalLink href={asset.href} label={`Open ${asset.title}`} location="agents_resource_open" target="_blank" rel="noreferrer" className="inline-text-cta type-p4">{agentsContent.resources.openCta}</TrackedExternalLink></div></div></li>)}</ul>
            </div>
          </section>
