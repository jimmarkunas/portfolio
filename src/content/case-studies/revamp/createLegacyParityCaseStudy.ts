...
const relatedImage = (slug: string) => `/${slug}/hero-${slug === "dtv02" ? "dtv02-01" : slug === "modere" ? "modere-01" : slug === "bi" ? "bi-01" : slug === "lego" ? "lego-01" : slug === "foh" ? "foh-01" : "dtv-01"}.png`
...
export function createLegacyParityCaseStudy(config: LegacyParityConfig): CaseStudyRevampData {
  ...
  const result: CaseStudyRevampData = {
    ...
    executiveBrief: {
      ...
      role: legacy.role.title,
      client: valueFor("Client"),
      timeline: valueFor("Timeline"),
      teamStakeholders: legacy.role.stats.find((stat) => /team|stakeholder/i.test(stat.label))?.value ?? "",
      budgetScale: legacy.role.stats.find((stat) => /budget/i.test(stat.label))?.value ?? "",
      systemsTechnology: legacy.problem.tools.map((tool) => tool.label).join(", "),
