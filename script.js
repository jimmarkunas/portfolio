...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ── Case Template Delivery Timeline: scroll progress + active dots ──
    const caseTemplateDeliveryTimeline = document.body.classList.contains('page-case-template')
        ? document.querySelector('#delivery .timeline')
        : null;

    if (caseTemplateDeliveryTimeline) {
        const deliveryTimelineItems = Array.from(caseTemplateDeliveryTimeline.querySelectorAll('.timeline-item'));
        let deliveryTimelineRaf = null;

        const updateDeliveryTimelineProgress = () => {
            const timelineRect = caseTemplateDeliveryTimeline.getBoundingClientRect();
            const viewportHeight = window.innerHeight || document.documentElement.clientHeight;
            const revealProbe = viewportHeight * 0.62;
            let progress = revealProbe - timelineRect.top;
            progress = Math.max(0, Math.min(progress, timelineRect.height));
            caseTemplateDeliveryTimeline.style.setProperty('--timeline-progress', `${progress}px`);

            const timelineStyles = window.getComputedStyle(caseTemplateDeliveryTimeline);
            const dotTop = parseFloat(timelineStyles.getPropertyValue('--timeline-dot-top')) || 4;
            const dotSize = parseFloat(timelineStyles.getPropertyValue('--timeline-dot-size')) || 16;
            const dotCenterOffset = dotTop + (dotSize / 2);

            deliveryTimelineItems.forEach(item => {
                const dotCenterY = item.offsetTop + dotCenterOffset;
                item.classList.toggle('is-active', progress >= dotCenterY);
            });
        };

        const queueDeliveryTimelineProgress = () => {
            if (deliveryTimelineRaf) return;
            deliveryTimelineRaf = requestAnimationFrame(() => {
                updateDeliveryTimelineProgress();
                deliveryTimelineRaf = null;
            });
        };

        updateDeliveryTimelineProgress();
        window.addEventListener('scroll', queueDeliveryTimelineProgress, { passive: true });
        window.addEventListener('resize', queueDeliveryTimelineProgress);
        window.addEventListener('load', queueDeliveryTimelineProgress);
    }
    ...
    if (anchorNav && anchorLinks.length > 0) {
        const isCaseTemplatePage = document.body.classList.contains('page-case-template');
        let anchorNavPlaceholder = null;

        if (isCaseTemplatePage) {
            anchorNavPlaceholder = document.createElement('div');
            anchorNavPlaceholder.className = 'anchor-nav-placeholder';
            anchorNav.insertAdjacentElement('afterend', anchorNavPlaceholder);
        }
        ...
        if (navItems.length > 0) {
            ...
            // Case-template hero keeps anchor nav inside #overview; this pins it beyond the section bounds.
            let caseTemplateAnchorStart = 0;
            const updateCaseTemplateAnchorPinning = () => {
                if (!isCaseTemplatePage || !anchorNavPlaceholder) return;

                const header = document.querySelector('header');
                const headerHeight = header ? Math.round(header.getBoundingClientRect().height) : 0;
                anchorNav.style.setProperty('--anchor-nav-top', `${headerHeight}px`);

                const shouldPin = window.scrollY + headerHeight >= caseTemplateAnchorStart;
                anchorNav.classList.toggle('anchor-nav--fixed', shouldPin);

                anchorNavPlaceholder.style.height = shouldPin ? `${anchorNav.getBoundingClientRect().height}px` : '0px';
            };

            const refreshCaseTemplateAnchorStart = () => {
                if (!isCaseTemplatePage) return;
                const wasFixed = anchorNav.classList.contains('anchor-nav--fixed');
                if (wasFixed) anchorNav.classList.remove('anchor-nav--fixed');

                caseTemplateAnchorStart = Math.round(anchorNav.getBoundingClientRect().top + window.scrollY);

                if (wasFixed) anchorNav.classList.add('anchor-nav--fixed');
                updateCaseTemplateAnchorPinning();
            };
            ...
            const updateAnchorSpy = () => {
                const probeOffset = getStickyOffset() + 4;
                ...
                navItems.forEach(({ hash, section }) => {
                    ...
                    // Prefer the section that currently spans the sticky probe line.
                    ...
                    // Fallback: nearest section above probe line.
                    if (!matchedSection && rect.top <= probeOffset) {
                        active = hash;
                    }
                });
            ...
            window.addEventListener('resize', () => {
                refreshCaseTemplateAnchorStart();
                queueAnchorSpy();
            });
            window.addEventListener('hashchange', queueAnchorSpy);

            if (isCaseTemplatePage) {
                window.addEventListener('scroll', updateCaseTemplateAnchorPinning, { passive: true });
                window.addEventListener('load', refreshCaseTemplateAnchorStart);
                refreshCaseTemplateAnchorStart();
            }
