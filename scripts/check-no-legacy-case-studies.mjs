...
const previewRegistry = fs.readFileSync(path.join(root, "src/content/case-studies/revamp/preview-registry.ts"), "utf8")
...
for (const slug of slugs) {
  if (!previewRegistry.includes(`record("${slug}"`)) errors.push(`${slug}: missing from live registry source`)
...
if (!liveRegistry.includes("caseStudyPreviewRegistry")) errors.push("live registry is not derived from preview registry")
